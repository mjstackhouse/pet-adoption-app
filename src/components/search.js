...
export default function Search() {
  ...
  return (
    <form method='get' action={`/search/${animalType}/${state}/${city}`} className='flex flex-wrap items-center justify-center max-w-[100%]'>
      <select id='select' className='basis-full text-black mx-4 mb-4 sm:mr-4 px-3 py-1 rounded-xl border-black border-2' value={animalType} onChange={e => setAnimalType(e.target.value)} required>
        <option value=''>Select an animal type</option>
        <option value='dog'>Dogs</option>
        <option value='cat'>Cats</option>
      </select>
      <input id='autocomplete' className='basis-full max-w-[90%] text-black mx-4 mb-4 sm:mr-4 px-3 py-1 rounded-xl border-black border-2' required></input>
      <button className='bg-blue text-black px-4 py-2 rounded-xl' type='submit' onClick={handleSubmit}>Search</button>
    </form>
  )
}