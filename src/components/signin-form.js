...
export default function SigninForm() {
  ...
  return (
    <div className='max-h-[60%] sm:max-w-[50vw] flex flex-wrap justify-start items-center bg-white text-base max-w-fit mx-4 sm:mx-8 px-4 py-6 sm:px-6 sm:py-8 rounded-3xl shadow-lg'>
      <div className='basis-full text-center mb-6'>
        <button className='font-bold tracking-wider bg-blue hover:bg-darker-blue hover:shadow-md text-black px-4 py-2 rounded-3xl mb-4 sm:mr-2' onClick={() => signIn('google', { callbackUrl })}>
          <FontAwesomeIcon icon={faGoogle} className='h-[1rem] mr-2' />
          Continue with Google
        </button>
        <button className='font-bold tracking-wider bg-blue hover:bg-darker-blue hover:shadow-md text-black px-4 py-2 rounded-3xl sm:ml-2' onClick={() => signIn('facebook', { callbackUrl })}>
      ...
      <form method='post' className='basis-full flex flex-wrap justify-start items-center'>
        ...
        <button className='font-bold tracking-wider hover:bg-darker-gray hover:shadow-md bg-white border-2 text-black px-4 py-2 mx-auto rounded-3xl' onClick={() => signIn('email', { email: `${userEmail}` })}>
