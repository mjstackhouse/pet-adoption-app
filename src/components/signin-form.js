  ...
  return (
    <div className='signin-form-container md:max-w-[700px] lg:max-w-[850px] md:mx-auto flex flex-wrap place-content-center bg-white text-base max-w-fit px-4 py-6 sm:px-6 sm:py-8 shadow-lg hidden'>
      ...
      <form method='post' className='basis-full flex flex-wrap justify-start items-center'>
        ...
        <button className='group font-bold tracking-wider hover:bg-darker-gray hover:underline underline-offset-4 hover:shadow-md bg-white border-2 text-black px-4 py-2 mx-auto rounded-3xl' onClick={() => signIn('email', { email: `${userEmail}` })}>
          <FontAwesomeIcon icon={faEnvelope} className='h-[1rem] mr-2 group-hover:scale-125 transition-transform' />
          Continue with email
        </button>
      </form>
    </div>
