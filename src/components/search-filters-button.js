...
export default function SearchFiltersButton({ parameters, breeds, searchParameters }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('resize', () => {
      ...
      else $('#filters-container').css('display', 'block');
    })
  ...
  return (
    <div className='xl:bg-gray fixed xl:sticky xl:self-start xl:h-auto top-[0] sm:top-[90px] xl:top-0 left-0 p-2 sm:p-8 md:p-4 xl:p-0 z-20 xl:z-10'>
      <div id='filters-container' className='bg-gray xl:pt-8 absolute xl:static top-0 bottom-0 left-0 right-0 mx-auto h-[100vh] xl:h-auto w-[100vw] xl:w-[315px] hidden xl:block z-10'>
        ...
        <form className='flex flex-wrap'>
          ...
          <div className='basis-full sm:mx-auto'>
            <div className='relative min-w-[250px] max-w-[90%] height-[50px] mb-8 mx-auto group'>
          ...
          <div className='basis-full sm:mx-auto'>
            <div className='relative min-w-[250px] max-w-[90%] height-[50px] mb-8 mx-auto group'>
              <select id='age' className='appearance-none w-full text-base h-[2.5rem] bg-white basis-full text-black px-3 py-1 rounded-3xl group-hover:brightness-90' value={age} onChange={e => {setAge(e.target.value); addToQueryParams('age', e.target.value);}} required>
          ...
          <div className='basis-full sm:mx-auto'>
            <div className='relative min-w-[250px] max-w-[90%] height-[50px] mb-8 mx-auto group'>
              <select id='size' className='appearance-none w-full text-base h-[2.5rem] bg-white basis-full text-black px-3 py-1 rounded-3xl group-hover:brightness-90' value={size} onChange={e => {setSize(e.target.value); addToQueryParams('size', e.target.value);}} required>
      ...
      <button onClick={() => openFilters()} className='flex my-20 sm:m-0 xl:hidden xl:pointer-events-none items-center bg-yellow hover:bg-darker-yellow rounded-lg sm:rounded-3xl font-bold tracking-wider hover:underline underline-offset-4 text-black px-4 py-2 mx-auto shadow-md group'>
