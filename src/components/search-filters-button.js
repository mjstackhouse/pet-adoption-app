...
export default function SearchFiltersButton({ parameters, breeds, searchParameters }) {
  ...
  return (
    <div className='xl:bg-darker-blue fixed xl:sticky xl:self-start xl:min-h-screen top-[0] sm:top-[90px] xl:top-0 left-0 p-2 sm:p-8 sm:p-4 xl:p-0 z-20'>
      <div id='filters-container' className='bg-darker-blue xl:bg-transparent xl:pt-8 absolute xl:static top-0 bottom-0 left-0 right-0 mx-auto h-[100vh] xl:h-auto w-[100vw] xl:w-[315px] hidden xl:block z-10'>
        ...
        <form className='flex flex-wrap'>
          <label htmlFor='breed' className={`basis-full max-w-[90%] text-left text-white font-bold tracking-wider mx-auto mb-2 pl-3`}>Breed</label>
          ...
          <label htmlFor='age' className={`basis-full max-w-[90%] text-left text-white font-bold tracking-wider mx-auto mb-2 pl-3`}>Age</label>
          <div className='basis-full sm:mx-auto'>
            <div className='relative min-w-[250px] max-w-[90%] height-[50px] mb-8 mx-auto'>
          ...
          <label htmlFor='size' className={`basis-full max-w-[90%] text-left text-white font-bold tracking-wider mx-auto mb-2 pl-3`}>Size</label>
          <div className='basis-full sm:mx-auto'>
            <div className='relative min-w-[250px] max-w-[90%] height-[50px] mb-8 mx-auto'>
          ...
          <div className='basis-full max-w-[90%] mx-auto mb-8 flex justify-left	items-center pl-3'>
            <span className='relative h-[1.5rem] w-[1.5rem] mr-4'>
              <input id='house_trained' type='checkbox' className='w-full h-full absolute accent-yellow' name='house_trained' value={houseTrained} onChange={e => { { e.target.value === 'false' ? setHouseTrained('true') : setHouseTrained('false')}; { e.target.value === 'false' ? addToQueryParams('house_trained', 'true') : addToQueryParams('house_trained', 'false')};}} />
            </span>
            ...
            <label for='house_trained' className={`inline text-left text-white font-bold tracking-wider`}>House-Trained</label>
          </div>
          <Link id='update-filters-btn' href={ Object.keys(queryParams).length !== 0 ? { query: queryParams } : `/search/${parameters.pets}/${parameters.state}/${parameters.city}` } passHref shallow onClick={() => { closeFilters(); scrollToStart(); }} className='flex items-center group font-bold tracking-wider hover:underline underline-offset-4 bg-yellow hover:bg-darker-yellow text-black px-4 py-2 mx-auto rounded-3xl hover:shadow-md mb-4'>
            <FontAwesomeIcon icon={faSliders} className='mr-2 h-[2rem] sm:h-[1.5rem] text-black group-hover:scale-110 transition-transform'/>
            Update filters
          </Link>
        </form>
      </div>
